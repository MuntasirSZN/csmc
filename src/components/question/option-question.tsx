    ...
    <div className="space-y-4">
      ...
      {answerType === 'single'
        ? (
            ...
            >
              {options.map(option => (
                <div key={`${id}-option-${option}`} className="flex items-center space-x-2 border p-3 rounded-md hover:bg-muted/50">
                  <RadioGroupItem value={option} id={`option-${id}-${option}`} />
                  <Label htmlFor={`option-${id}-${option}`} className="flex-1 cursor-pointer">
        ...
        : (
            <div className="space-y-4 text-left">
              {options.map((option) => {
                ...
                return (
                  <div key={`${id}-checkbox-${option}`} className="flex items-start space-x-2 border p-3 rounded-md hover:bg-muted/50">
                    <Checkbox
                      id={`option-${id}-${option}`}
                      checked={isChecked}
